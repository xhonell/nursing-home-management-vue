...
<script>
import { handleDoctorLeave, updateDoctorLeave } from "@/api/leave";
...
export default {
  ...
  methods: {
    getList() {
      handleDoctorLeave().then(response => {
    ...
    approveLeave() {
      // 同意请假的逻辑
      updateDoctorLeave(this.selectedLeaveItem.leaveId,"同意申请").then(response => {
        this.$message({
    ...
    rejectLeave() {
      // 驳回请假的逻辑
      updateDoctorLeave(this.selectedLeaveItem.leaveId,"驳回申请").then(response => {
        this.$message({
          message: response.msg,
          type: 'success'
        });
